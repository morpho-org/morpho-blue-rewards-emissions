...
contract BlueRewardsProgramRegistry is Multicall {
    ...
    /// @notice Returns the time-bounded rewards programs for a given id.
    /// Where id = keccak256(abi.encode(msg.sender, urd, rewardToken, market)).
    mapping(bytes32 programId => RewardsProgram[MAX_PROGRAMS_WITH_SAME_ID]) public rewardsPrograms;

    /// @notice Emitted when a rewards program is registered.
    ...
    function getPrograms(address caller, address urd, address rewardToken, Id market)
        public
        view
        returns (RewardsProgram[MAX_PROGRAMS_WITH_SAME_ID] memory)
    {
